"""端到端测试 v2 - Step 2: AI 自动生成角色 - 验证是否保留 宋霄/钱开凤"""
import json
...
try:
    with urllib.request.urlopen(req, timeout=300) as resp:
        ...
        for c in result.get("characters", []):
            ...
            print(f"    Identity: {c.get('identity', '')[:80]}")
            print(f"    Personality: {c.get('personality', '')[:80]}")
            print(f"    Initial power: {c.get('initial_power', '')[:80]}")
            print()
